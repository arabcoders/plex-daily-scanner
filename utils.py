...
def get_files(input: pathlib.Path, recursive: bool, extensions: list = [],
              sideCar: bool = True) -> list[dict["file": pathlib.Path, "sidecar": list[pathlib.Path]]]:
    """
    ...
    if input.is_file():
        return [
            {
                "file": input,
                "sidecar": getSideCarFiles(input) if sideCar else []
            }
        ]

    def natural_sort(l):
        def convert(text): return int(text) if text.isdigit() else text.lower()
        def alphanum_key(key): return [convert(c) for c in re.split('([0-9]+)', str(key))]
        return sorted(l, key=alphanum_key)

    files: list = []

    for filename in natural_sort([item for item in input.glob('**/*.*' if recursive else '*.*')]):
        ...
        files.append({
            "file": file,
            "sidecar": getSideCarFiles(file) if sideCar else []
        })
...
def spread_mtime(files: list[dict["file":pathlib.Path, "sidecar":list[pathlib.Path]]]) -> None:
    for file in files:
        ...
        os.utime(file['file'], (mod_time, mod_time,))
        LOG.info(
...
def fix_id(files: list[dict["file":pathlib.Path, "sidecar":list[pathlib.Path]]]) -> None:
    """
...
def cli():
    ...
    opt_input.add_argument("-r", "--recursive", action="store_true", help="Recursively get files.")
    ...
    opt_options.add_argument("-m", "--min-files", type=int, help=f"Number of minium sidecar files.", default=2)
    opt_options.add_argument("-sm", "--spread-mtime", action="store_true", help='Increase file mod_time.')
    opt_options.add_argument("-ui", "--update-id", action="store_true", help='Update Ids.')
    ...
    files = get_files(pathlib.Path(args.get('input')), args.get('recursive'), args.get('extensions'))
    ...
    for file in files:
        ...
        if len(file['sidecar']) >= minFiles:
            LOG.debug(f"File: {file['file']}, has '{minFiles}' or more sidecar files:\n{file['sidecar']}")
            continue
