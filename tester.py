...
def cli():
    ...
    for file in files:
        ...
        for rg in RX_LIST:
            ...
            if match:
                LOG.debug(
                    f"File: '{c(file['file'],'yellow')}' matched: '{c(rg.pattern,'yellow')}'"
                )
                id = get_date(match, file["file"])
                if id:
                    matched += 1
                    LOG.info(
                        f"File: '{c(file['file'],'yellow')}' id: '{c(id,'yellow')}'"
                    )
                else:
                    unmatched += 1
        ...
        if not match:
            unmatched += 1
            LOG.warning(
                f"File: '{c(file['file'],'yellow')}' did not match: '{c(rg.pattern,'yellow')}'"
            )
